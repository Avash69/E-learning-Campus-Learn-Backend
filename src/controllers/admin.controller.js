...
const adminSignUp = asyncHandler(async (req, res) => {
    ...
    if (!newAdmin) throw new ApiError(500, "Admin creation failed");

    return res.status(200).json(new ApiResponse(200, {}, "Admin added successfully"));
});
...
const adminLogin = asyncHandler(async (req, res) => {
    ...
    const loggedAdmin = await admin.findOne({ username });
    if (!loggedAdmin) throw new ApiError(400, "Admin does not exist");
    ...
    const safeAdmin = await admin.findById(loggedAdmin._id).select("-password -Refreshtoken");
    ...
    return res
        ...
        .cookie("Refreshtoken", Refreshtoken, options)
        .status(200)
        .json(new ApiResponse(200, { admin: safeAdmin }, "Logged in successfully"));
});
...
const forApproval = asyncHandler(async (req, res) => {
    ...
    const teachers = await Teacher.find({ Isverified: true });

    return res
        .status(200)
        .json(new ApiResponse(200, { admin: loggedAdmin, studentsforApproval: students, teachersforApproval: teachers }, "Fetched successfully"));
});
...
const approveStudent = asyncHandler(async (req, res) => {
    ...
    const { Isapproved, email, remarks = null } = req.body;

    if (!["approved", "rejected", "reupload"].includes(Isapproved)) {
        throw new ApiError(400, "Choose approved | rejected | reupload");
    }
    ...
    if (!updatedStudent) throw new ApiError(400, "Student not found");

    await Sendmail(email, "Document Verification Status",
        `<html>
            <body>
                <h2>Document Verification Status</h2>
                <p>Your verification status: <strong>${Isapproved}</strong></p>
                <p>Remarks: ${remarks}</p>
            </body>
        </html>`
    );

    return res.status(200).json(new ApiResponse(200, updatedStudent, "Task completed"));
});
...
const approveTeacher = asyncHandler(async (req, res) => {
    ...
    if (!updatedTeacher) throw new ApiError(400, "Teacher not found");
    ...
    return res.status(200).json(new ApiResponse(200, updatedTeacher, "Task completed"));
});
...
const checkStudentDocuments = asyncHandler(async (req, res) => {
    ...
    const theStudent = await student.findById(studentID);
    if (!theStudent) throw new ApiError(400, "Student not found");

    const docs = await studentdocs.findById(theStudent.Studentdetails);
    if (!docs) throw new ApiError(400, "Documents not found");

    return res.status(200).json(new ApiResponse(200, { loggedAdmin, theStudent, studentDocs: docs }, "Fetched"));
});
...
const checkTeacherDocuments = asyncHandler(async (req, res) => {
    ...
    const theTeacher = await Teacher.findById(teacherID);
    if (!theTeacher) throw new ApiError(400, "Teacher not found");

    const docs = await Teacherdocs.findById(theTeacher.Teacherdetails);
    if (!docs) throw new ApiError(400, "Documents not found");

    return res.status(200).json(new ApiResponse(200, { loggedAdmin, theTeacher, teacherDocs: docs }, "Fetched"));
});
...
const sendmessage = asyncHandler(async (req, res) => {
    ...
    if ([name, message, email].some((f) => !f || f.trim() === "")) {
        throw new ApiError(400, "All fields are required");
    }

    const saved = await contact.create({ name, email, message });

    return res.status(200).json(new ApiResponse(200, saved, "Message sent"));
});
...
const readMessage = asyncHandler(async (req, res) => {
    ...
    const updated = await contact.findByIdAndUpdate(messageID, { status: true }, { new: true });

    if (!updated) throw new ApiError(400, "Message update failed");
...
const toapproveCourse = asyncHandler(async (req, res) => {
    ...
    const courses = await course.find({ isapproved: false }).populate("enrolledteacher");
...
const approveCourse = asyncHandler(async (req, res) => {
    ...
    const { Isapproved, email, Firstname } = req.body;

    // APPROVE COURSE
    if (Isapproved === true || Isapproved === "true") {
        const updated = await course.findByIdAndUpdate(courseID, { isapproved: true }, { new: true });

        await Sendmail(email, "Course Approved",
            `<html><body>
                <h2>Course Approved!</h2>
                <p>Hello ${Firstname}, your course (${updated.coursename}) is approved.</p>
            </body></html>`
        );

        return res.status(200).json(new ApiResponse(200, updated, "Course approved"));
    }

    // REJECT COURSE
    const deleted = await course.findByIdAndDelete(courseID);

    await Sendmail(email, "Course Rejected",
        `<html><body>
            <h2>Course Rejected</h2>
            <p>Hello ${Firstname}, your course (${deleted?.coursename ?? "Course"}) has been rejected.</p>
        </body></html>`
    );

    return res.status(200).json(new ApiResponse(200, {}, "Course rejected"));
});
